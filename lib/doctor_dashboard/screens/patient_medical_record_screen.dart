...
class PatientMedicalRecordScreen extends StatefulWidget {
  ...
  final Map<String, String>? preFilledData; // Add preFilledData parameter
  ...
  const PatientMedicalRecordScreen({
    ...
    this.appointmentId,
    this.preFilledData, // Add to constructor
  });
...
class _PatientMedicalRecordScreenState
  ...
  List<Map<String, dynamic>> _radiologyReports =
      []; // Add list for radiology reports
  ...
  void initState() {
    ...
    if (widget.preFilledData != null) {
      _conditionController.text =
          widget.preFilledData!['medical_condition'] ?? '';
      _symptomsController.text = widget.preFilledData!['symptoms'] ?? '';
      _notesController.text = ''; // Always keep notes empty
      _medicationsController.text = widget.preFilledData!['medications'] ?? '';
      _testsController.text = widget.preFilledData!['tests'] ?? '';

      // Show the new medical record dialog immediately
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text('Error loading medical records: ${e.toString()}')),
        );
        setState(() {
          _isLoadingRecords = false;
        });
  ...
  Future<void> _loadLabReports() async {
    try {
      ...
      print(
          'Attempting to load lab reports for patient ID: ${widget.patientId}');

      // First, get the reports from the database
      final response = await _supabase
          .from('lab_reports')
      ...
      // Transform the response but don't modify the report_url as it's already a public URL
      final transformedReports = List<Map<String, dynamic>>.from(response);

      print('Transformed reports: $transformedReports');

      setState(() {
        _labReports = transformedReports;
        _isLoadingLabReports = false;
      });
    } catch (e) {
      print('Error in _loadLabReports: ${e.toString()}');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _loadMedications() async {
    try {
      ...
      print(
          'Attempting to load medications for patient ID: ${widget.patientId}'); // Log start
      final response = await _supabase
          .from(
              'medications') // Assuming your medications table is named 'medications'
          .select(
              '*') // Select all columns, including medication_id if it exists
          .eq('patient_id', widget.patientId)
          .neq('status',
              'cancelled') // Add filter to exclude cancelled medications
          .order('created_at',
              ascending: false); // Assuming a 'created_at' column for ordering

      print(
          'Supabase medications response: ${response.length} records found.'); // Log success
      print(
          'Supabase medications fetched data (first 5 records): ${response.take(5).toList()}'); // Log fetched data
      ...
      print(
          'setState called after loading medications. isLoadingMedications: $_isLoadingMedications'); // Log setState call
    } catch (e) {
  ...
  Future<void> _loadRadiologyReports() async {
    try {
      ...
      print(
          'Attempting to load radiology reports for patient ID: ${widget.patientId}');
      ...
      setState(() {
        _radiologyReports = transformedReports;
        _isLoadingRadiology = false;
      });
    } catch (e) {
      print('Error in _loadRadiologyReports: ${e.toString()}');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content:
  ...
  Future<void> _updateMedicalRecord(
    ...
    try {
      ...
      print(
          'Data: {medical_condition: $medicalCondition, symptoms: $symptoms, notes: $notes, medications: $medications, tests: $tests}');
  ...
  Future<void> _deleteMedicalRecord(String recordId) async {
    print(
        '[_deleteMedicalRecord] Called with recordId: $recordId'); // Added logging
    if (recordId.isEmpty) {
      print(
          '[_deleteMedicalRecord] recordId is empty. Aborting delete.'); // Added logging
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
    ...
    try {
      ...
      print(
          '[_deleteMedicalRecord] Dialog result: $confirmDelete'); // Added logging

      // If user confirmed deletion
      if (confirmDelete == true) {
        print(
            '[_deleteMedicalRecord] User confirmed deletion.'); // Added logging
        final response = await _supabase
            .from('medical_records')
            .delete()
        ...
        print(
            '[_deleteMedicalRecord] Supabase delete response: $response'); // Added logging
        ...
        print(
            '[_deleteMedicalRecord] User cancelled deletion.'); // Added logging
      }
    } catch (e) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text('Error deleting medical record: ${e.toString()}')),
        );
      }
    }
  }
            ...
            children: [
              Expanded(
                child: OutlinedButton.icon(
                  onPressed: () {
                    _showEditMedicalRecordDialog(
                        context, record); // Call edit dialog function
                  },
              ...
              Expanded(
                child: OutlinedButton.icon(
                  onPressed: () {
                    print(
                        'Delete button pressed for record ID: $recordId'); // Added logging
                    if (recordId.isNotEmpty) {
                      _deleteMedicalRecord(
                          recordId); // Call delete function with record ID
                    } else {
                      print(
                          'Record ID is empty, cannot delete'); // Added logging
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                            content: Text(
                                'Cannot delete record: Invalid record ID')),
                      );
                    }
                  },
        ...
        if (selectedLabRadiologyTab == 1) // Radiology
          Padding(
            ...
            child: SizedBox(
              ...
              child: ElevatedButton.icon(
                onPressed: () => _showAddLabRadiologyRecordDialog(context,
                    isRadiology: true), // Pass type
                icon: const Icon(Icons.add),
                label: const Text('Create New Record'),
                style: ElevatedButton.styleFrom(
                  backgroundColor: theme.colorScheme
                      .primary, // Use primary color for radiology button
                  foregroundColor: Colors.white,
                  padding: const EdgeInsets.symmetric(vertical: 14),
                  shape: RoundedRectangleBorder(
                    borderRadius: BorderRadius.circular(30),
                    ...
                    : _labReports.isEmpty
                        ...
                        : ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              print(
                                  '[buildLabAndRadiology - Lab] Data before building card: ${report}');
                              ...
                              Color statusColor =
                                  AppTheme.greyColor; // Default color
                              ...
                              final String doctorName =
                                  report['doctors']?['name'] ?? 'N/A';

                              // Assuming 'report_id' is the primary key for lab_reports
                              final String reportId = report['report_id'] ??
                                  ''; // Handle potential null by providing empty string
                              print(
                                  '[LabReportCard] Report ID: $reportId'); // Added logging

                              // Add logging to show the full report data before building the card
                              print(
                                  '[buildLabAndRadiology] Building Report Card for: $report');

                              return _buildReportCard(
                                'Lab Report',
                                {
                                  ...report, // Pass the complete original report data
                                  'title': title,
                                  'date': date,
                                  'status': status,
                    ...
                    : _radiologyReports.isEmpty
                        ...
                        : ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              print(
                                  '[buildLabAndRadiology - Radiology] Data before building card: ${report}'); // **Added logging**
                              print(
                                  '[buildLabAndRadiology] Processing Radiology Report index $index: $report'); // **Added logging**
                              ...
                              final String reportId =
                                  report['Radiology_id']?.toString() ?? '';
                              print(
                                  '[RadiologyReportCard] Report ID: $reportId'); // Added logging

                              // Add logging to show the full report data before building the card
                              print(
                                  '[buildLabAndRadiology] Building Radiology Report Card for: $report');
                              ...
                              return _buildReportCard(
                                'Radiology Report', // Explicitly label as Radiology Report
                                {
                                  ...
                                  'statusColor': statusColor,
                                  'result':
                                      status, // Using status for result as per existing structure
                                  'desc':
                                      'Report added by Dr. $doctorName', // Assuming similar description structure
                                  'report_url': report[
                                      'report_url'], // Add the report_url from the database
                                  'Title':
                                      title, // Add Title field for consistency
                                  'created_at': report[
                                      'created_at'], // Add created_at for proper date handling
                                  'doctors': {
                                    'name': doctorName
                                  }, // Add doctors object for consistency
                                  'Radiology_id': report[
                                      'Radiology_id'], // Pass the Radiology_id directly from the original report
                                },
                              );
                            },
                          ))),
      ...
      child: Column(
        ...
        children: [
          Icon(Icons.receipt_long_outlined,
              size: 60, color: theme.colorScheme.onSurface.withOpacity(0.5)),
          const SizedBox(height: 18),
          Text('No lab or radiology reports found',
              style: theme.textTheme.bodyLarge?.copyWith(
                  color: theme.colorScheme.onSurface.withOpacity(0.7))),
          const SizedBox(height: 8),
          Text(
    ...
    return _medicationsList.isEmpty
        ...
        : ListView.builder(
            ...
            itemBuilder: (context, index) => _buildMedicationCard(
                _medicationsList[index]), // Use loaded data
          );
  }

  Widget _buildReportCard(String type, Map<String, dynamic> report) {
    print(
        '[buildReportCard] Building card for type: $type, data: $report'); // Debug log
    ...
    print('[buildReportCard] Determined recordId: $recordId'); // Debug log
    print('[buildReportCard] Report URL: ${report['report_url']}'); // Debug log
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(
              'Date: ${report['created_at'] != null ? DateFormat('dd MMM, yyyy').format(DateTime.parse(report['created_at']).toLocal()) : 'N/A'}',
              style: AppTheme.bodyMedium.copyWith(color: AppTheme.greyColor)),
          const SizedBox(height: 8),
          Row(
            children: [
          ...
          if (report['doctors']?['name'] != null) ...[
            const SizedBox(height: 8),
            Text('Doctor: Dr. ${report['doctors']['name']}',
                style: AppTheme.bodyMedium.copyWith(color: AppTheme.greyColor)),
          ],
          ...
          Row(
            ...
            children: [
              Expanded(
                child: OutlinedButton.icon(
                  onPressed: () {
                    print('Report data before viewing:');
                    print(report);
              ...
              Expanded(
                child: OutlinedButton.icon(
                  onPressed: () {
                    _showEditLabRadiologyRecordDialog(context, report,
                        isRadiology: type == 'Radiology Report',
                        recordId: recordId);
                  },
                  icon: const Icon(Icons.edit, color: AppTheme.primaryColor),
                  label: Text('Edit',
              ...
              Expanded(
                child: OutlinedButton.icon(
                  onPressed: () {
                    if (recordId.isNotEmpty) {
                      _deleteLabRadiologyRecord(recordId,
                          isRadiology: type == 'Radiology Report');
                    } else {
                      print(
                          '[buildReportCard] Cannot delete: recordId is empty.');
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                            content: Text(
                                'Cannot delete record: Invalid record ID')),
  ...
  Widget _buildMedicationCard(Map<String, dynamic> medication) {
    ...
    if (startDateString != null && endDateString != null) {
      try {
        ...
        durationText =
            '${duration.inDays + 1} days'; // Add 1 to include both start and end days
      } catch (e) {
    ...
    final String medicationId =
        medication['medication_id'] ?? ''; // Handle potential null
              ...
              Expanded(
                child: OutlinedButton.icon(
                  onPressed: () {
                    ...
                    print(
                        '[MedicationCard] Edit button pressed for: ${medication['medication_id']}'); // Added logging
                    _showEditMedicationDialog(
                        context, medication); // Call edit dialog
                  },
                  icon: const Icon(Icons.edit, color: AppTheme.primaryColor),
                  label: Text('Edit',
                      style: AppTheme.bodyLarge
              ...
              Expanded(
                child: OutlinedButton.icon(
                  onPressed: () {
                    ...
                    print(
                        '[MedicationCard] Delete button pressed for ID: $medicationId'); // Added logging
                    if (medicationId.isNotEmpty) {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                            content: Text(
                                'Cannot delete medication: Invalid medication ID.')),
                      );
                      print(
                          '[MedicationCard] Cannot delete: medicationId is empty.'); // Added logging
                    }
                  },
                  icon: const Icon(Icons.delete_outline, color: Colors.red),
                  label: Text('Delete',
  ...
  Future<void> _deleteMedication(String medicationId) async {
    print(
        '[_deleteMedication] Called with medicationId: $medicationId'); // Added logging
    if (medicationId.isEmpty) {
      print(
          '[_deleteMedication] medicationId is empty. Aborting delete.'); // Added logging
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content:
    ...
    try {
      ...
      print(
          '[_deleteMedication] Dialog result: $confirmDelete'); // Added logging
      ...
      if (confirmDelete == true) {
        ...
        print(
            '[_deleteMedication] Supabase delete response: $response'); // Added logging

        // Check if the delete was successful (Supabase might not throw an error on no rows deleted)
        if (response == null || response.isEmpty) {
          // Assuming response is null or empty on successful delete
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                  content: Text('Medication record deleted successfully')),
            );
          }
          print('[_deleteMedication] Delete successful.'); // Added logging
        } else {
          // This case might indicate an error depending on Supabase client
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                  content: Text(
                      'Failed to delete medication record: ${response.toString()}')),
            );
          }
          print(
              '[_deleteMedication] Delete failed. Response: ${response}'); // Added logging
        }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content:
                  Text('Error deleting medication record: ${e.toString()}')),
        );
      }
    }
  }
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                Text(
                  isRadiology
                      ? 'New Radiology Record'
                      : 'New Lab Record', // Change dialog title
                ...
                TextField(
                  ...
                  decoration: themedInputDecoration(
                    label: isRadiology
                        ? 'Radiology Title'
                        : 'Report Title', // Change hint text
                    icon: isRadiology
                        ? Icons.receipt_long_outlined
                        : Icons.description_outlined, // Change icon
                  ),
                  style: AppTheme.bodyLarge,
                ),
                const SizedBox(height: 16),
                ...
                TextFormField(
                  ...
                  controller: TextEditingController(
                      // Use a temporary controller for display
                  ...
                  onTap: () async {
                    final DateTime? picked = await showDatePicker(
                      ...
                      lastDate: DateTime(
                          2101), // Allow selecting dates up to the year 2100
                    );
                    if (picked != null && picked != selectedDate) {
                      setState(() {
                        selectedDate = picked;
                        print(
                            'Selected date: $selectedDate'); // Keep for debugging
                      });
                    }
                  },
                ),
                      ...
                      child: ElevatedButton(
                        onPressed: () {
                          ...
                          if (title.isNotEmpty &&
                            ...
                            _addLabRadiologyRecord(title, date, status,
                                isRadiology); // Call the save function
                          } else {
  ...
  Future<void> _addLabRadiologyRecord(
    ...
    try {
      if (isRadiology) {
        ...
        print(
            '[_addLabRadiologyRecord] Radiology record saved to DB: Title: $title, Date: $date, Status: $status');
        return;
      }
  ...
  Future<void> _deleteLabRadiologyRecord(String reportId,
    ...
    try {
      ...
      final bool isRadiologyRecord = _radiologyReports.any((r) =>
          r['Radiology_id']?.toString() ==
          reportId); // **Corrected to use Radiology_id**
      print(
          '[_deleteLabRadiologyRecord] Attempting to delete record with ID: $reportId'); // Added logging
      print(
          '[_deleteLabRadiologyRecord] isRadiologyRecord: $isRadiologyRecord'); // Added logging

      await _supabase
          .from(isRadiologyRecord
              ? 'Radiology'
              : 'lab_reports') // Select table based on flag
          .delete()
          .eq(isRadiologyRecord ? 'Radiology_id' : 'report_id',
              reportId); // **Corrected to use Radiology_id for Radiology table**
  ...
  void _showEditLabRadiologyRecordDialog(
    ...
    final String currentRecordId = recordId ??
        ...
        ''; // Use passed recordId, fallback to map
    print(
        '[EditLabRadiologyDialog] Using record ID: $currentRecordId'); // Added logging
    ...
    print(
        '[EditLabRadiologyDialog] isRadiologyRecord: $isRadiologyRecord'); // Added logging
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                Text(
                  isRadiologyRecord
                      ? 'Edit Radiology Record'
                      : 'Edit Lab Record', // Dynamic title
                  style: AppTheme.titleLarge
                      .copyWith(color: AppTheme.primaryColor),
                ),
                const SizedBox(height: 16),
                  ...
                  onTap: () async {
                    ...
                    if (picked != null && picked != selectedDate) {
                      setState(() {
                        selectedDate = picked;
                        print(
                            '[EditLabRadiologyDialog] Date selected: $selectedDate'); // Added logging
                      });
                    }
                  },
                ),
                ...
                DropdownButtonFormField<String>(
                  ...
                  onChanged: (String? newValue) {
                    setState(() {
                      selectedStatus = newValue;
                      print(
                          '[EditLabRadiologyDialog] Status selected: $selectedStatus'); // Added logging
                    });
                  },
                  items: <Map<String, dynamic>>[
                    {'value': 'Normal Results', 'color': Colors.green},
                    ...
                    Expanded(
                      child: ElevatedButton(
                        onPressed: () async {
                          ...
                          print(
                              '[EditLabRadiologyDialog] Save button pressed.');
                          print(
                              '[EditLabRadiiologyDialog] updatedTitle: $updatedTitle');
                          print(
                              '[EditLabRadiologyDialog] updatedDate: $updatedDate');
                          print(
                              '[EditLabRadiologyDialog] updatedStatus: $updatedStatus');
                          print(
                              '[EditLabRadiologyDialog] recordId: $currentRecordId'); // Use the correctly determined ID
                          ...
                          if (currentRecordId.isNotEmpty &&
                            ...
                            // Show validation error
                            print(
                                '[EditLabRadiologyDialog] Validation failed.'); // Added logging
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
                                  content: Text(
  ...
  Future<void> _updateLabRadiologyRecord(
    ...
    try {
      // Add logging to check the data being sent
      print(
          '[_updateLabRadiologyRecord] Attempting to update lab/radiology record with ID: $recordId');
      print(
          '[_updateLabRadiologyRecord] isRadiology: $isRadiology'); // Added logging
      print(
          '[_updateLabRadiologyRecord] Data: {Title: $title, created_at: ${date.toIso8601String()}, status: $status}');
      ...
      print(
          '  Update Data: {Title: $title, created_at: ${date.toIso8601String()}, status: $status}');
      ...
      final response = await _supabase
          ...
          .eq(isRadiology ? 'Radiology_id' : 'report_id',
              recordId) // **Corrected to use Radiology_id for Radiology table**
          .select(); // Select the updated row to check the response

      // Add logging for the Supabase response
      print(
          '[_updateLabRadiologyRecord] Supabase update response: ${response}');
      ...
      if (response.isEmpty) {
        ...
        print(
            '[_updateLabRadiologyRecord] Update failed: response is empty.'); // Added logging
        return; // Exit if no record was updated
      }
      ...
      print(
          '[_updateLabRadiologyRecord] Update successful. Loading reports...'); // Added logging
      ...
      print(
          '[_updateLabRadiologyRecord] Error: ${e.toString()}'); // Added logging
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(
  ...
  void _showNewMedicalRecordDialog(BuildContext context) {
    ...
    if (widget.preFilledData != null) {
      _conditionController.text =
          widget.preFilledData!['medical_condition'] ?? '';
      _symptomsController.text = widget.preFilledData!['symptoms'] ?? '';
      _notesController.text = ''; // Force notes to be empty
      _medicationsController.text = widget.preFilledData!['medications'] ?? '';
      _testsController.text = widget.preFilledData!['tests'] ?? '';
    } else {
      // Initialize all controllers as empty
      _conditionController.text = '';
          ...
          child: Column(
            ...
            children: [
              ...
              TextField(
                ...
                decoration: themedInputDecoration(
                    label: 'Medications', icon: Icons.medical_services),
                style: AppTheme.bodyLarge,
              ),
              const SizedBox(height: 16),
              TextField(
                ...
                decoration:
                    themedInputDecoration(label: 'Tests', icon: Icons.science),
                style: AppTheme.bodyLarge,
              ),
              const SizedBox(height: 24),
              Row(
                    ...
                    child: ElevatedButton(
                      onPressed: () async {
                        ...
                        final String notes = _notesController.text
                            .trim(); // This will be empty by default
                    ...
                    child: ElevatedButton(
                      onPressed: () async {
                        ...
                        final String updatedMedications =
                            _medicationsController.text.trim();
                        final String updatedTests =
                            _testsController.text.trim();

                        // Update the record in the database
                        await _updateMedicalRecord(
                          record[
                              'record_id'], // Assuming 'record_id' is the primary key
                          updatedCondition,
                          updatedSymptoms,
                          updatedNotes,
                          updatedMedications,
  ...
  void _showEditMedicationDialog(
    ...
    DateTime? selectedStartDate = medication['start_date'] != null
        ? DateTime.tryParse(medication['start_date'])
        : null;
    DateTime? selectedEndDate = medication['end_date'] != null
        ? DateTime.tryParse(medication['end_date'])
        : null;

    // Get the status value from the medication record and log it
    String? selectedStatus = medication['status'] ?? null;
    print(
        '[EditMedicationDialog] Status from record: ${medication['status']}'); // Added logging
    print(
        '[EditMedicationDialog] Initializing selectedStatus with: $selectedStatus'); // Added logging

    final String medicationId = medication['medication_id']?.toString() ?? '';

    print(
        '[EditMedicationDialog] Initial medication data: $medication'); // Added logging
    print(
        '[EditMedicationDialog] Initial medicationId: $medicationId'); // Added logging
            ...
            child: Column(
              ...
              children: [
                ...
                TextField(
                  ...
                  decoration: themedInputDecoration(
                      label: 'Dosage', icon: Icons.medical_services),
                  style: AppTheme.bodyLarge,
                ),
                const SizedBox(height: 16),
                // Frequency Field
                TextField(
                  ...
                  decoration: themedInputDecoration(
                      label: 'Frequency', icon: Icons.repeat),
                  style: AppTheme.bodyLarge,
                ),
                const SizedBox(height: 16),
                ...
                TextFormField(
                  ...
                  decoration: themedInputDecoration(
                      label: 'Start Date', icon: Icons.calendar_today_outlined),
                  style: AppTheme.bodyLarge,
                  onTap: () async {
                    final DateTime? picked = await showDatePicker(
                      context: context,
                      ...
                      firstDate:
                          DateTime(2000), // Allow past dates for start date
                    ...
                    if (picked != null && picked != selectedStartDate) {
                      setState(() {
                        ...
                        print(
                            '[EditMedicationDialog] Start Date selected: $selectedStartDate'); // Added logging
                      });
                    }
                  },
                ),
                ...
                TextFormField(
                  ...
                  decoration: themedInputDecoration(
                      label: 'End Date', icon: Icons.calendar_today_outlined),
                  style: AppTheme.bodyLarge,
                  onTap: () async {
                    final DateTime? picked = await showDatePicker(
                      context: context,
                      ...
                      firstDate: selectedStartDate ??
                          DateTime
                              .now(), // End date cannot be before start date
                      lastDate: DateTime(2101),
                    );
                    if (picked != null && picked != selectedEndDate) {
                      setState(() {
                        ...
                        print(
                            '[EditMedicationDialog] End Date selected: $selectedEndDate'); // Added logging
                      });
                    }
                  },
                ),
                ...
                TextField(
                  ...
                  maxLines: 3,
                  decoration: themedInputDecoration(
                      label: 'Notes', icon: Icons.note_alt),
                  style: AppTheme.bodyLarge,
                ),
                const SizedBox(height: 16),
                // Status Dropdown
                DropdownButtonFormField<String>(
                  ...
                  onChanged: (String? newValue) {
                    setState(() {
                      selectedStatus = newValue;
                      print(
                          '[EditMedicationDialog] Status selected: $selectedStatus'); // Added logging
                    });
                  },
                  items: <String>['active', 'completed', 'canceled']
                    ...
                    return DropdownMenuItem<String>(
                      value: value,
                      child: Text(value
                          .capitalize()), // Assuming you have a capitalize extension or function
                    );
                  }).toList(),
                ),
                const SizedBox(height: 24),
                    ...
                    Expanded(
                      child: ElevatedButton(
                        onPressed: () async {
                          ...
                          print(
                              '[EditMedicationDialog] medicationId: $medicationId');
                          print(
                              '[EditMedicationDialog] updatedName: $updatedName');
                          print(
                              '[EditMedicationDialog] updatedDosage: $updatedDosage');
                          print(
                              '[EditMedicationDialog] updatedFrequency: $updatedFrequency');
                          print(
                              '[EditMedicationDialog] updatedStartDate: $updatedStartDate');
                          print(
                              '[EditMedicationDialog] updatedEndDate: $updatedEndDate');
                          print(
                              '[EditMedicationDialog] updatedStatus: $updatedStatus');

                          if (medicationId.isNotEmpty &&
                              updatedName.isNotEmpty &&
                              updatedDosage.isNotEmpty &&
                              updatedFrequency.isNotEmpty &&
                            ...
                            // Show validation error
                            print(
                                '[EditMedicationDialog] Validation failed.'); // Added logging
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
                                  content: Text(
                                      'Please fill all required fields and ensure medication has a valid ID')),
                            );
                          }
                        },
                        style: ElevatedButton.styleFrom(
  ...
  Future<void> _updateMedication(
    ...
    try {
      // Add logging to check the data being sent
      print(
          '[_updateMedication] Attempting to update medication with ID: $medicationId');
      print(
          '[_updateMedication] Data: {name: $name, dosage: $dosage, frequency: $frequency, start_date: ${startDate.toIso8601String()}, end_date: ${endDate.toIso8601String()}, notes: $notes, status: $status}');
      ...
      final response = await _supabase
          ...
          .update({
            ...
            'status': status,
            'updated_at':
                DateTime.now().toIso8601String(), // Update updated_at timestamp
          })
          .eq('medication_id', medicationId)
          .select(); // Select the updated row to check the response
      ...
      if (response.isEmpty) {
        ...
        print(
            '[_updateMedication] Update failed: response is empty.'); // Added logging
        return; // Exit if no record was updated
      }
      ...
      print(
          '[_updateMedication] Update successful. Loading medications...'); // Added logging
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
              color:
                  theme.colorScheme.primary), // Add border for unselected state
        ),
        child: Center(
          child: Text(
            label,
            style: theme.textTheme.bodyLarge?.copyWith(
              color: selected
                  ? theme.colorScheme.onPrimary
                  : theme.colorScheme.primary, // Text color based on selection
              fontWeight: FontWeight.w600,
              fontSize: 14,
            ),
            overflow: TextOverflow.ellipsis,
